...
function DataProvenance({ data }: { data: CrawlerAnalyticsResponse }) {
  return <>
    <p>{copy.source}</p>
    <p>{copy.totalRequestBaseline}：{data.summary.totalRequests}</p>
    <p>{copy.queryWindow}：{data.meta.start} 至 {data.meta.end}</p>
    <p>{copy.lastRefresh}：{data.meta.generatedAt}</p>
    {data.meta.sampled ? <p>{copy.sampled}</p> : null}
...
function DashboardData({ data }: { data: CrawlerAnalyticsResponse }) {
  const cards = [
    [copy.automationTotal, data.summary.crawlerRequests, data.summary.totalRequests],
  ...
  return <div className="mt-8 space-y-10">
    ...
    <DataTable title={copy.agents}><table className={tableClass}><thead className="text-muted-foreground"><tr><th className={cellClass}>名称</th><th className={cellClass}>{copy.declaredUserAgent}</th><th className={cellClass}>{copy.category}</th><th className={cellClass}>{copy.requests}</th></tr></thead><tbody>{data.agents.map((row) => <tr key={`${row.name}-${row.userAgent}`}><td className={cellClass}>{row.name}</td><td className={cellClass}>{row.userAgent}</td><td className={cellClass}>{copy.categories[row.category]}</td><td className={cellClass}>{row.requests}</td></tr>)}</tbody></table></DataTable>
