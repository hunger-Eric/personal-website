...
function IdentityPreview({ preview }: { preview: CrawlerIdentityPreview }) {
  ...
  return <section aria-labelledby="identity-preview-title" className="border-t border-hairline pt-5">
    ...
    {preview.chinaUaCoverage ? <div className="mt-8"><DataTable title={copy.identityPreview.chinaCoverage.title}><p className="mb-4 max-w-3xl text-sm leading-6 text-muted-foreground">{copy.identityPreview.chinaCoverage.description}</p><table aria-label={copy.identityPreview.chinaCoverage.title} className={tableClass}><thead className="text-muted-foreground"><tr><th className={cellClass}>名称</th><th className={cellClass}>{copy.identityPreview.chinaCoverage.provider}</th><th className={cellClass}>{copy.identityPreview.chinaCoverage.token}</th><th className={cellClass}>{copy.identityPreview.chinaCoverage.purpose}</th><th className={cellClass}>{copy.identityPreview.chinaCoverage.verification}</th></tr></thead><tbody>{preview.chinaUaCoverage.map((row) => <tr key={row.id}><td className={cellClass}>{row.name}</td><td className={cellClass}>{row.providerName}</td><td className={`${cellClass} font-mono text-xs`}>{row.uaToken}</td><td className={cellClass}>{copy.identityPreview.chinaCoverage.purposes[row.purpose]}</td><td className={cellClass}>{copy.identityPreview.statuses[row.verificationStatus]}</td></tr>)}</tbody></table></DataTable></div> : null}
  </section>;
}
...
function HumanTraffic({ human }: { human: NonNullable<CrawlerAnalyticsResponse["human"]> }) {
  ...
  const breakdowns = human.devices?.length && human.browsers?.length && human.operatingSystems?.length ? [
  ...
  const locations = human.countries?.length && human.regions?.length ? {
    countries: human.countries,
    regions: human.regions,
  } : null;
