    ...
    <div className="overflow-x-hidden bg-surface-paper text-surface-paper-foreground">
      ...
      <section id="capabilities" className="mx-auto max-w-6xl px-4 py-16 lg:py-20">
        ...
        <div className="grid gap-px border-x border-b border-hairline bg-hairline sm:grid-cols-2">
          {content.service.capabilities.map((item, index) => {
            const Icon = capabilityIcons[item.id];
            return (
              <article key={item.id} className="flex flex-col bg-surface-paper p-6 sm:p-8">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs text-accent">0{index + 1}</span>
                  <Icon className="h-5 w-5 text-foreground" aria-hidden />
                </div>
                <h3 className="mt-10 text-xl font-semibold text-foreground">{item.title}</h3>
                <p className="mt-3 max-w-md text-sm leading-7 text-muted-foreground">{item.description}</p>
                <p className="mb-5 mt-5 border-t border-hairline pt-4 text-sm leading-7 text-foreground">
                  <span className="font-semibold">{zh ? "常见场景：" : "Typical situation: "}</span>
                  {item.example}
                </p>
                <Link href={`${path("/services")}#scenarios`} className="mt-auto inline-flex min-h-11 items-center gap-2 self-start text-sm font-semibold text-accent hover:text-accent-hover">
                  {zh ? "查看相关场景与项目" : "Explore scenarios and projects"}
