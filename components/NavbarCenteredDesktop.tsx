  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 hidden border-b border-hairline bg-surface-paper/95 backdrop-blur sm:block">
      <div className="mx-auto flex h-18 max-w-6xl items-center justify-between px-4">
        <Link href="/" className="flex items-baseline gap-2 text-foreground"><span className="text-lg font-semibold tracking-[-0.03em]">实解智能</span><span className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{copy.subtitle}</span></Link>
        ...
        <div className="flex items-center gap-3"><LangSwitch variant="ghost" /><Link href="/contact" className="inline-flex items-center gap-2 bg-accent px-4 py-2.5 text-sm font-semibold text-accent-foreground transition-colors hover:bg-accent-hover">{copy.contact}<ArrowUpRight className="h-4 w-4" aria-hidden /></Link></div>
      </div>
    </header>
  );
}
