...
export async function generateMetadata({params}:Props):Promise<Metadata>{const{id}=await params;const project=getPublicWebsiteProject(id,"zh");if(!project)return{};return buildPublicPageMetadata({title:`${project.name}：${project.category}`,description:project.summary,path:`/projects/${project.id}`})}
