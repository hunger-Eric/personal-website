...
export const EDITORIAL_TASK_PROMPTS = {
  ...
  article_source_bound_write: 'Return exactly this top-level JSON shape and no other keys: {"title":"string","slugProposal":"kebab-case","summary":"string","tags":["string"],"body":"Chinese Markdown with adjacent [[S001]] citations","sourceAssessments":[{"sourceId":"S001","category":"official|standard|original_research|peer_reviewed","rationale":"string","claimsSupported":["string"]}]}. Write one coherent Chinese business article of 1200-1800 Chinese characters from the exact editorialBrief and supplied evidence. Open with a concrete problem, fact, or judgment. Every paragraph must advance a new fact, action, distinction, or consequence; use natural headings only. Cite every supplied source ID in the body at least once. Every prose paragraph of 60 or more characters must contain a supplied source citation. Every substantive external claim needs an adjacent supplied source citation. Return exactly one source assessment for every supplied source ID, using each exact ID once. If the thesis cannot be supported, fail rather than revise it in code.',
} as const;
